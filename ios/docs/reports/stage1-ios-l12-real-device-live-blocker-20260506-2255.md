# Stage 1 iOS L12 Real-Device Live Blocker - 2026-05-06 22:55 CST
...
for the physical iPhone 12-family real-device validation gate. This batch
therefore refreshed the smallest real iOS validation evidence available on this
machine and reran both required physical-device probes.

## Validation Commands

| Command from `ios/` | Result | Evidence |
| --- | --- | --- |
| `swift test` | PASS | Built `FastMDMobile` and executed 219 XCTest cases with 0 failures and 0 unexpected failures in 16.470 seconds. Swift Testing reported 0 tests in 0 suites passed. |
...
| `xcodebuild -scheme FastMDMobile -destination 'platform=iOS Simulator,name=iPhone 12' test` | PASS | Xcode resolved the SwiftPM package scheme, ran on the iPhone 12 simulator destination, and reported `** TEST SUCCEEDED **`; 219 XCTest cases executed with 1 skipped and 0 failures. |
| `xcrun xctrace list devices` | BLOCKED for physical iPhone 12-family validation | Probe completed. It listed the Mac host, offline physical iPhone/iPad devices, and simulator destinations. The iPhone 12 entry was a simulator, not a connected physical device. |
