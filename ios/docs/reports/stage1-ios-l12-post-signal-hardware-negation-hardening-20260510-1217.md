# Stage 1 iOS L12 Post-Signal Hardware Negation Hardening

- Generated local: 2026-05-10 12:17:28 CST
- Generated UTC: 2026-05-10T04:17:28Z
...
connected physical iPhone 12 / 12 mini / iPhone 12 Pro / iPhone 12 Pro Max.
...
The L12 manual real-device evidence parser now rejects post-signal negation for
iPhone 12-family hardware tokens. Evidence that names a physical iPhone
12-family signal and then says that signal was unavailable, offline, unverified,
not verified, disconnected, or otherwise negated no longer satisfies either:
...
- `testIOSL12RealDeviceValidationRejectsPostSignalUnavailableHardwareEvidence`
- `testIOSL12RealDeviceValidationRejectsPostSignalNegatedConnectedHardwareMatch`

These complement the existing prefix-negation tests and keep the real-device
gate fail-closed when blocker language appears after the hardware token.

## Validation Commands

| Command | Working directory | Result | Evidence |
| --- | --- | --- | --- |
| `swift test --filter FastMDMobileCoreTests/testIOSL12RealDeviceValidationRejectsPostSignal` | `ios/` | PASS | 2 selected XCTest cases, 0 failures. |
| `swift test --filter FastMDMobileCoreTests/testIOSL12` | `ios/` | PASS | 99 selected XCTest cases, 0 failures, 0 unexpected failures. |
| `swift test` | `ios/` | PASS | 274 XCTest cases, 0 failures, 0 unexpected failures, 66.320s test time. |
...
- `ios/docs/reports/stage1-ios-l12-post-signal-hardware-negation-hardening-20260510-1217.md`
