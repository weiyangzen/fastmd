...
real iOS validation supported by the current SwiftPM skeleton plus fresh
simulator and physical-device probes, then recorded redacted evidence under
`ios/docs/reports/`.
...
non-iPhone-12-family hardware, missing required probe commands, and missing
manual flow evidence do not complete the physical gate.

## Validation Commands

| Command | Result | Evidence |
| --- | --- | --- |
| `swift test` from `ios/` | PASS | 216 XCTest cases, 0 failures, 0 unexpected failures, 15.699 seconds test execution time. |
