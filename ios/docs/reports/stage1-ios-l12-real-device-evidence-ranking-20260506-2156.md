# Stage 1 iOS L12 Real-Device Evidence Ranking - 2026-05-06 21:56 CST
...
This batch hardened the iOS real-device validation report when duplicate manual
evidence rows exist for the same required flow step.

Changed behavior:

- Real-device manual evidence row selection now prefers evidence that references
  the verified connected iPhone 12-family hardware signal.
- It then prefers evidence observed after the current physical-device probe.
- It still retains the existing preference for physical iPhone 12-family
  evidence and current non-stale evidence.

This prevents a report row from displaying an older or mismatched manual summary
when a stronger current post-probe row exists for the connected verified device.

## Validation Results

| Command | Result | Evidence |
| --- | --- | --- |
| `swift test` from `ios/` | PASS | Built the SwiftPM package and executed 217 XCTest cases with 0 failures and 0 unexpected failures in 15.706 seconds of test execution time. Swift Testing reported 0 tests in 0 suites passed. |
