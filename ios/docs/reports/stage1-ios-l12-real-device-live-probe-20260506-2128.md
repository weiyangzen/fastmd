...
The earliest still-open iOS-owned checklist item is the physical iPhone
12-family real-device validation gate. This bounded batch reran the smallest
real iOS validation supported by the current SwiftPM skeleton, refreshed
simulator and physical-device probes, and recorded redacted evidence under
`ios/docs/reports/`.

No Swift source change was needed in this batch. The current iOS validation
contracts already fail closed for simulator-only evidence, unavailable physical
devices, non-iPhone-12-family hardware, missing probe commands, and missing
manual Stage 1 flow evidence.

## Validation Commands

| Command | Result | Evidence |
| --- | --- | --- |
| `swift test` from `ios/` | PASS | Build completed and 216 XCTest cases passed with 0 failures and 0 unexpected failures in 15.606 seconds of test execution time. |
...
| `xctrace` | iPad | offline physical device | iPad physical device, not connected for validation | no |
