...
- Date: 2026-05-06 21:07 Asia/Shanghai
...
required local iOS validation plus current simulator and physical-device probes,
then recorded the result under `ios/docs/reports/` without editing Android or
root Docs files.

No native Swift source change was needed in this batch. The existing iOS
validation model remains fail-closed: simulator evidence, unavailable devices,
...
| --- | --- | --- |
| `swift test` from `ios/` | PASS | 216 XCTest cases, 0 failures, 0 unexpected failures, 16.046 seconds in the `FastMDMobileCoreTests` suite. |
...
Private device identifiers, serial numbers, and hostnames are intentionally not
copied into this report. The gate decision is based only on device class,
connection state, simulator/physical reality, and hardware family signal.
