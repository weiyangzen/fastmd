...
- Generated local: 2026-05-10 09:49:24 CST
- Generated UTC: 2026-05-10T01:49:41Z
...
iOS-owned implementation row. The remaining iOS-owned open row is:
...
| --- | --- | --- | --- |
| `swift test` | `ios/` | PASS | Build complete; 264 XCTest cases executed, 0 failures, 0 unexpected failures, 42.890s test time. |
...
| `xcodebuild -scheme FastMDMobile -destination 'platform=iOS Simulator,name=iPhone 12' test` | `ios/` | PASS | `** TEST SUCCEEDED **`; 264 XCTest cases executed, 1 skipped, 0 failures, 0 unexpected failures, 20.737s test time. |
| `xcrun xctrace list devices` | `ios/` | PASS command, BLOCKED physical gate | Probe exited 0 and listed the Mac host, unavailable physical iOS/iPadOS records, and simulator destinations. The exact `iPhone 12` entry appeared only under simulators, not as connected physical hardware. |
...
- `ios/docs/reports/stage1-ios-l12-live-validation-batch-20260510-0949.md`
