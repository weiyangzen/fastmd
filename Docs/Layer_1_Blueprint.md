...
- The macOS host must expose a menu-bar status-item switch for the app-level monitoring state, matching the tray / status-indicator responsibility that Windows and Ubuntu hosts will also carry in their own shells.
...
- The first small top-right preview control must be a `Pin` affordance for the current window. Preview windows start unpinned by default. Clicking an external `http` / `https` link must open the destination in a new FastMD window, automatically pin the old window, and leave the new window unpinned. Pin means the window should not disappear automatically, but pin must not change the ability to drag and reposition the window.
