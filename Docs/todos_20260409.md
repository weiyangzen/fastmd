...
Worker 2 note (2026-04-09): the hidden desktop-shell evidence lane now supports an explicit Ubuntu review-signoff export that writes `Docs/Test_Logs/ubuntu-validation-review-signoff.{md,json}` and only flips the hidden cross-session evidence state to ready after the latest Wayland and X11 reports are both individually ready and explicitly reviewed. This item still remains open until those real-machine captures exist and the blueprint/todo are updated honestly.
