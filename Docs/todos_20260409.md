...
_Worker 2 note (2026-04-09): the hidden Ubuntu evidence lane now distinguishes `cross-session-captured-awaiting-review` from `cross-session-review-stale`, exposes review-artifact freshness booleans in hidden shell metadata, and writes the explicit cross-session review status into exported Ubuntu validation reports so real Wayland/X11 reviewers can tell whether a saved sign-off is missing, stale, or current without prematurely closing the Layer 7 parity-evidence item._
