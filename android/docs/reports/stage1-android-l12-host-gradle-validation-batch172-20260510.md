# Stage 1 Android L12 Host Gradle Validation Batch 172 - 2026-05-10
...
- `android/docs/reports/stage1-android-l12-host-gradle-validation-batch172-20260510.md`

Gradle also refreshed Android-local generated build metadata, lint reports, test
reports, and APK/AAR outputs under generated `build/` directories while running
validation.

## Environment

Commands were run from `/Users/wangweiyang/GitHub/fastmd/android` on
2026-05-10 around 07:45 CST.
...
| --- | --- | --- |
| `date '+%Y-%m-%d %H:%M:%S %Z %z'` | PASS | Printed `2026-05-10 07:45:15 CST +0800`. |
...
| `./gradlew lint build :core:testDebugUnitTest :feature:reader:testDebugUnitTest :app:assembleDebug --no-daemon --console=plain -Pfastmd.useChinaMavenMirror=true` with explicit JDK 17 and SDK env | PASS | `BUILD SUCCESSFUL in 2m 9s`; `474 actionable tasks: 14 executed, 460 up-to-date`. |
| `adb devices -l` | BLOCKED for connected validation | Printed only `List of devices attached`; no device rows. |
| `emulator -list-avds` from default shell path | BLOCKED | `/bin/bash: emulator: command not found`. |
| `/Users/wangweiyang/Library/Android/sdk/emulator/emulator -list-avds` | PASS for preflight only | Listed `Medium_Phone`. |
