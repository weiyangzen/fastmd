# Stage 1 Android L12 Host Validation Refresh - Batch 164
...
batch refreshed the host-side Gradle validation gates that do not require an
...
- `android/docs/reports/stage1-android-l12-host-validation-refresh-batch164-20260510.md`
...
- Time captured before validation: `2026-05-10 06:34:16 CST +0800`.
- Default shell `java -version`: blocked by macOS with
  `Unable to locate a Java Runtime`.
- Default shell `./gradlew --no-daemon projects`: blocked by the same missing
...
| `./gradlew --no-daemon projects` | BLOCKED | The default shell wrapper invocation hit the same missing Java runtime before Gradle started. |
