# Stage 1 Android L12 Validation Pass - Batch 151
...
validation open. This bounded batch refreshed the earliest Android-owned host
validation items that can run on the current machine:
...
This batch also attempted `:app:connectedDebugAndroidTest` once to capture the
current device blocker. No Android product source changes were required. No
`ios/**`, shared `Docs/**`, or `.cron/**` files were edited.

## Changed Android Files

- `android/docs/reports/stage1-android-l12-validation-pass-batch151-20260510.md`
...
This was not a blocker for the selected Gradle host gates because every Gradle
command pinned `JAVA_HOME` to the explicit OpenJDK 17 installation above.

## Validation Results

All passing Gradle commands below were run with this environment prefix:
...
| --- | --- | --- |
| `java -version` | BLOCKED in default shell | macOS reported no discoverable Java runtime. |
| `JAVA_HOME=... java -version` | PASS | Reported OpenJDK `17.0.17`. |
| `./gradlew projects` | PASS | Root project `fastmd-android` evaluated with modules `:app`, `:core`, `:feature:library`, `:feature:reader`, and `:feature:settings`; `BUILD SUCCESSFUL in 13s`. |
| `./gradlew :core:testDebugUnitTest :feature:reader:testDebugUnitTest :app:assembleDebug stage1AndroidPerformanceReport` | PASS | Core and reader debug unit tests, debug app assembly, and source-level performance capture completed; `BUILD SUCCESSFUL in 19s`. |
| `./gradlew lint build` | PASS | Module lint, debug/release build paths, debug/release unit tests, and Stage 1 renderer asset/request-blocking gates completed; `BUILD SUCCESSFUL in 2m 19s`; `474 actionable tasks: 14 executed, 460 up-to-date`. |
| `./gradlew :app:connectedDebugAndroidTest` | BLOCKED | App and androidTest APK preparation completed, then task execution failed with `DeviceException: No connected devices!`; `BUILD FAILED in 20s`. |
| `adb devices -l` | BLOCKED for device validation | Printed `List of devices attached` with no device rows. |
| `emulator -list-avds` | PARTIAL | Printed `Medium_Phone`; no emulator was booted in this batch. |
| `sdkmanager --version` from expected cmdline-tools path | BLOCKED | `sdkmanager` is missing at `/Users/wangweiyang/Library/Android/sdk/cmdline-tools/latest/bin/sdkmanager`. |
