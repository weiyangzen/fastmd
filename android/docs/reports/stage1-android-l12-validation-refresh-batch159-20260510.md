# Stage 1 Android L12 Validation Refresh - Batch 159
...
refreshed the host-side Android validation gates that can run in the current
environment, then rechecked the connected/device validation blockers.
...
- `android/docs/reports/stage1-android-l12-validation-refresh-batch159-20260510.md`
...
- Time captured before validation: `2026-05-10 05:45:58 CST +0800`.
...
- `Medium_Phone` config observed: `target = android-36`, `abi.type =
  arm64-v8a`, `hw.lcd.width = 1080`, `hw.lcd.height = 2400`, `hw.ramSize =
  2048`, Play Store enabled, 16 KB page-size system image.
...
| `./gradlew --no-daemon --console=plain -Pfastmd.useChinaMavenMirror=true :app:connectedDebugAndroidTest` | BLOCKED | Debug app and androidTest APKs were prepared, then `:app:connectedDebugAndroidTest` failed with `DeviceException: No connected devices!`; `152 actionable tasks: 6 executed, 146 up-to-date`; `BUILD FAILED in 19s`. |
...
The current `testDebugUnitTest` XML results under the Android build directories
report zero failures/errors:
...
The combined host validation command exercised the Android Stage 1 renderer
